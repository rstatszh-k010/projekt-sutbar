...
# Die NA's bei der CVP und BDP werden auf 0 gesetzt, da der NA-Wert tatsächlich als 0 angesehen werden kann (Das Fehlen der Werte bedeutet inhaltlich 0, da es bei diesen zwei Parteien keine Nennungen gibt, da diese im Wahljahr 2023 nicht mehr exisitierten.)
...
# Danach wird eine Grafik erstellt, bei welcher man die Verteilung der Parteien nach Grösse auf einen Blick ersichtlich ist
...
```{r}
# Visualisierung 2: Summe Anzahl gewählte Frauen über alle Kantone und Parteien.
...
# Tabelle 2: Anzahl gewählte Frauen (mit gt und einer function(x)):
...
gewaehlte_f_prozent <- function(sum_anzahl_gewaehlte_f) {
  round(sum_anzahl_gewaehlte_f / sum(anzahl_gewaehlte, na.rm = TRUE) * 100, 1)
}
  
# Nun habe ich die Prozentwerte mittels einer function(x) definiert. Nun würde ich gerne eine schöne Tabelle machen mit den Werten: wahl_jahr, sum_anzahl_gewaehlte_f, gewaehlte_f_prozent. Aber beim Befehl mutate() gibt es mir eine Fehlermeldung aus. Warum?

NRW2023_zeitreihe_Kanton |> 
  filter(!is.na(anzahl_gewaehlte_f)) |> 
  group_by(wahl_jahr) |> 
  summarise(sum_anzahl_gewaehlte_f = sum(anzahl_gewaehlte_f)) |>
  mutate(prozent = gewaehlte_f_prozent(total)) |> 
  gt::gt()
...
```{r}
# Visualisierung 3: Entwicklung Frauen-Anteil pro Partei in einem Kanton, pro Partei
#(Vorschlag 2.1)
